                ...
                switch (nptr->tok) {
                    case TOK_PLUS:
                        if(nptr -> children[0] -> type == INT_TYPE){
                            ...
                            char *point = malloc(strlen(nptr->children[0]->val.sval) + strlen(nptr -> children[1] ->val.sval) + 1);
                            //nptr -> val.sval = ptr; 
                            point[0] = '\0'; 
                            strcat(point, nptr -> children[0] -> val.sval); 
                            strcat(point, nptr -> children[1] -> val.sval); 
                            nptr -> val.sval = point; 
                            free(point); 
